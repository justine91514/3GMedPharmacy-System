...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->Cell(0, 90, 'Supplier Report', 0, false, 'C', 0, '', 0, false, 'T', 'M');
...
$query = "SELECT * FROM supplier_list ORDER BY id DESC"; // Added ORDER BY id DESC
$result = mysqli_query($connection, $query);

// Calculate the maximum width needed for each column
$maxWidth = array(20, 30, 32, 30, 45); // Default widths for ID, Supplier Name, Contact Number, Address, and Agreement
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $maxWidth[1] = max($maxWidth[1], $pdf->GetStringWidth($row['supplier_name']) + 5);
    $maxWidth[2] = max($maxWidth[2], $pdf->GetStringWidth($row['contact']) + 5);
    $maxWidth[3] = max($maxWidth[3], $pdf->GetStringWidth($row['address']) + 5);
    $maxWidth[4] = max($maxWidth[4], $pdf->GetStringWidth($row['agreement']) + 5);
}
...
$pdf->Cell($maxWidth[1], 10, 'Supplier Name', 1, 0, 'C', 1);
$pdf->Cell($maxWidth[2], 10, 'Contact Number', 1, 0, 'C', 1);
$pdf->Cell($maxWidth[3], 10, 'Address', 1, 0, 'C', 1);
$pdf->Cell($maxWidth[4], 10, 'Agreement', 1, 1, 'C', 1);
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Cell($maxWidth[1], 10, $row['supplier_name'], 1, 0, 'C');
    $pdf->Cell($maxWidth[2], 10, $row['contact'], 1, 0, 'C');
    $pdf->Cell($maxWidth[3], 10, $row['address'], 1, 0, 'C');
    $pdf->Cell($maxWidth[4], 10, $row['agreement'], 1, 1, 'C');
    $pdf->SetX($headerXPos);
}
