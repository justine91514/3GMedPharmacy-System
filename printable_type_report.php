...
class MYPDF extends TCPDF {
    ...
    public function Header() {
        ...
        $this->Cell(0, 90, 'Type Report', 0, false, 'C', 0, '', 0, false, 'T', 'M');
...
$query = "SELECT * FROM product_type_list ORDER BY id DESC";
$result = mysqli_query($connection, $query);

// Calculate the maximum width needed for each column
$maxWidth = array(10, 70); // Default widths for ID and Type Name
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $maxWidth[1] = max($maxWidth[1], $pdf->GetStringWidth($row['type_name']) + 5);
}
...
$pdf->Cell($maxWidth[1], 10, 'Type Name', 1, 1, 'C', 1);
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Cell($maxWidth[1], 10, $row['type_name'], 1, 1, 'C');
    $pdf->SetX($headerXPos);
}
